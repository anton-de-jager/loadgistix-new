...
export class SettingsPlanBillingComponent implements OnInit {
    ...
    getPayfast(token) {
        let myData = {};
        // Merchant details
        myData["merchant-id"] = environment.merchant_id;
        myData["token"] = token;
        myData["version"] = 'v1';
        myData["timestamp"] = new Date().toISOString();

        // Arrange the array by key alphabetically for API calls
        let ordered_data = {};
        Object.keys(myData).sort().forEach(key => {
        ...
        setTimeout(() => {
            ...
            for (let key in myData) {
                getString += key + '=' + encodeURIComponent(myData[key]).replace(/%20/g, '+') + '&';
            }
